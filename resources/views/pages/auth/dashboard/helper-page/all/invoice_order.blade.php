        ...
        <div class="row">
            <div class="col">
                ...
                <div style="padding-top: 20px;">
                    <table class="table table-borderless">
                        <tr>
                            <td>Bill To:</td>
                            <td scope="col">{{ $fetch_order_details->name }}</td>
                        </tr>
                        <tr>
                            <td>Address:</td>
                            <td scope="col">
                                {{ $fetch_order_details->address }}<br>
                            </td>
                        </tr>
                        <tr>
                            <td>Contact:</td>
                            <td scope="col">{{ $fetch_order_details->phone }}</td>
                        </tr>
                    </table>
                </div>
            </div>
            <div class="col">
                <div class="float-end">
                    ...
                    <div style="color:#1c1c1c"># <span id="invoiceID"> {{ $fetch_order_details->oder_id }}</span></div>
                    <span class="text-white px-2" style="background-color: #0a7151;">Zone: {{ $fetch_order_details->zone }}</span>
                    <div style="padding-top: 10px;">
                        <p><span style="color:#1c1c1c">Date:</span><span style="padding-left:40px">{{ \Carbon\Carbon::createFromFormat('Y-m-d H:i:s',$fetch_order_details->created_at)->format('d-m-y') }}</span></p>
                    </div>
                    <div style="background-color:#f5f5f5;">
                        <p style="padding:5px;padding-left:10px"><span style="color:#1c1c1c">Total Bill:</span><span
                                style="padding-left:25px;color:#1c1c1c">{{ $fetch_order_details->total_price }} &#2547;</span>
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div class="col">
                <div style="padding-top: 40px;">
                    <table class="table table-bordered" style="width: 100%; border: 1px solid #0a7151;">
                        ...
                        <tbody>
                            @foreach(json_decode($fetch_order_details->products,true) as $key => $pd)
                            <tr>
                                <td>
                                    {{ strlen($pd['product_name']) > 40 ? substr($pd['product_name'], 0, 20) . '..' : $pd['product_name'] }}
                                </td>
                                <td>{{ $pd['single_product_weight'] }}</td>
                                <td>{{ $pd['total_quantity'] }}</td>
                                <td>{{ $pd['signle_price'] }}  &#2547;</td>
                            ...
                            <tr>
                                ...
                                <td>{{ $fetch_order_details->delivery_charge }} &#2547;</td>
                            </tr>
                            <tr>
                                <td colspan="4">Grand Total</td>
                                <td>{{ $fetch_order_details->total_price }} &#2547;</td>
                            </tr>
                        </tbody>
                    </table>
